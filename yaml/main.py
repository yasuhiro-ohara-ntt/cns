...
for node in data['nodes']:
    for i in range(len(node['interfaces'])):
        ...
        if (iface['type']=='direct'):
            ...
            # print('[{}] '.format(iface['type']), end='')
            # print('{}:{}'.format(node['name'],iface['name']), end='')
            # print(' pear={}:{}'.format(r_nname,r_ifname))
            peer_found = False
            for link in links:
...
for sw in data['switches']:
    for i in range(len(sw['interfaces'])):
        ...
        # print('{}: name={} type={} opts={}'.format(
        #     sw['name'], iface['name'], iface['type'], iface['opts']))
        for link in links:
